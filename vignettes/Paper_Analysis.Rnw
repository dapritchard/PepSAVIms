...
analysis performed in \papername/ (herafter abbreviated to \papername/).
Providing the analysis will hopefully make the research more reproducible, give
the interested reader a greater understanding of the methodology developed in
the paper, and to provide a working template for researchers who may wish to
apply the methodology developed in the paper to their own research problems.
...
analysis obtained 30,799 MS features for \textit{Viola odorata} across 34
fractions.
...
First we load the mass spectrometry data collected for \papername/; this is the
data produced by the Progenesis instrumentation and software.  The raw data
...
Readers who wish to obtain the data in its raw form as produced by the
Progenesis system, may obtain the data as well as the \R/ script used to convert
the raw data into the form provided with the package from the \pkg{PRISMS}
package developement repository located at
\url{https://github.com/dpritchLibre/PRISMS}.


\subsubsection{Mass spectrometry data}

The mass spectrometry data produced by the Progenesis system yielded intact mass
and relative intensity information for peptide constituents contained within
each library fraction; the analysis obtained 30,799 MS features for
\textit{Viola odorata} across fractions 1-43 and additionally for fraction 47.
The data is measured in units of Daltons.
...
in the \pkg{PRISMS} pipeline is to to consolidate mass spectrometry observations in
...
Discussion: why choose time range to include all, mass b/w 2000 and 15000,
charge range all, mtoz difference < 0.05, and time of peak retention to include
all?
...
Some discussion here.  Do we have a figure available here?   *************



\subsubsection{Filtering criteria selection}

Why was it chosen to have a max bordering ratio of 0.01, a minimum intensity in
region of interest of 1000, and and max charge of 10
