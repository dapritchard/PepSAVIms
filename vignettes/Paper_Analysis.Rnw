...
working template for researchers who may wish to use the methodology developed
in the paper and the tools developed in this package for their own research.
...
\subsection{\textit{Viola odorata}  liquid chrometography mass spectrometry} \label{sec: lcms}
...
\subsection{Bioactivity screening}
...
\ref{fig: flow chart}.  The blue nodes are functions in the \pkg{PRISMS}
ecosystem; the pink nodes denotes mass spectrometry data to be used as data
inputs; the green node denotes bioactivity data to be used as a data input; and
the yellow node denotes the analysis results of using the \pkg{PRISMS}
...
steps have already been performed using other tools, or in situations where some
of the data processing steps may not be appropriate for the particular data
analysis in hand.
...
A conception view of the functions in the \pkg{PRISMS} package is provided in
the following subsections.  Please refer to the function documentation for more
information regarding their application programming interface, as well as
...
same compound - in which case we wish to attribute all of the elution to this
compound and remove the appearance of having multiple compounds.  The function
name \binMS/ derives from the fact that we use a binning procedure to
...
\item Each observation must have a mass that falls within the specified interval
...
of the following criteria to belong to the same compound.
...
The next optional preprocessing step for the mass spectrometry abundance data is
a filtering step.  The goal of the filtering step is to reduce the list of
candidate compounds for reduction of bioactivity levels through subject-matter
knowledge prior to using statistical methods.  By filtering the candidate set
...
\item Compound charge state must be less than or equal to the specified value.
...
\rankEN/ function, and takes as inputs both the preprocessed mass
spectrometry abundance data and the bioactivity levels data.  The procedure
works by specifying the level of the $\ell_2$ penalty parameter in the elastic
net penalty, and tracking the inclusion of the coefficients corresponding to
compounds into the nonzero set along the elastic net path.  An ordered list of
candidate compounds for reduction of bioactivity levels is then obtained by
providing the order in which the coefficients corresponding to compounds entered
the nonzero set.
...
directly from user the without any preprocessing.




% Describing prototypical workflow -------------------------
...
analysis performed in \papername/, which is the workflow shown in Figure
...
First we describe the data analysis workflow as performed in \papername/, which
is the workflow shown in Figure \ref{fig: flow chart prototypical}.  In this
scenario, mass spectrometry abundance data across various fractions for the
biological analyte is obtained via a mass spectrometer, and is represented by
the light red oval leftmost in the top line of nodes (see section *** of *** for
the laboratory procedure used to procure this data).  Concurrently, bioactivity
data is obtained via ****, as described in section **** of ****; this data is
represented as the light green node alone on the second row of nodes.  The
bioactivity data is not immediately used; first some preprocessing steps are
performed on the mass spectrometry data.

<<reading-data>>=
# Read the data
@

The first step performed is to consolidate the mass spectometry abundances that
are presumed to belong to the same chemical compound; we refer to this process
as binning the data, where the term binning is derived from the method used to
achieve this aim.  This task is accomplished be passing the mass spectrometry
data as an argument to the \R/ function \binMS/.

Next we wish to perform some feature screening to reduce the level of noise in
the data by removing any potential candidate compounds with observed abundances
for which it is not scientifically plausible that they might be a compound with
an effect on bioactivity levels (see section **** of ****).  This task is
performed by passing the \R/ data object created by \binMS/ and passing it
as an argument to \filterMS/.

At this point we have mass spectrometry data for which the original chemical
compounds that had been spread out over multiple observations recovered by the
binning process, and implausible compounds screened out of the dataset by the
filtering process, as well as the corresponding bioactivity data.  Now it
remains to perform a statistical procedure by which we can obtain a list of
candidate compounds which may have a (promotive) effect on the reduction of
bioactivity levels in the bioactivity data.  This is accomplished by passing the
\R/ object returned by the \filterMS/ data in addition to the bioactivity
data to the \filterMS/ function.  The \R/ object returned by this function
provides such a list of candidate compounds.
