...
understanding of the methodology developed in the paper, and provide a working
template for researchers who may wish to apply the methodology developed in the
paper to their own research problems.
...
greater detail regarding this process. \vspace{-2mm}
...
coupled to an AB Sciex TripleTOF5600).  After processing of this dataset,
...
\textit{Viola odorata} across the 34 fractions.
...
breast cancer (bc), prostate cancer (pc) and ovarian cancer (oc). Library fractions are
incubated with a microbial or cancer cell culture and the presence of bioactive
peptides in a given fraction will result in inhibition of culture growth during
the incubation period.  For bacterial assays, the remaining viable cells are
quantified indirectly by spectrophotometric measurement of the irreversible
intracellular reduction of resazurin25.  For anticancer bioactivity,
cytotoxicity assays are performed using MTT-based assays to measure
...
data analysis of course remain unchanged from their original form.
...
of F. graminearum, which had two replicates), across fractions 1-43.  The
...
breast cancer (bc), prostate cancer (pc) and ovarian cancer (oc). Each region of activity is defined using blue % activity bars,
...
and thereby restrict attention to potential compounds of interest. Retention
...
consolidation step required choices to be made in specifying the criteria by
which features are determined to belong to the same compound.  An m/z difference
of 0.05 Da was chosen for our data analysis to be the maximum difference for
which two compounds could have in m/z levels to be considered to belong to the
same compound.  The value of 0.05 Da was chosen by performing multiple runs of
the MS-LC data collection process and studying the variation in the
...
difference in Da and had the same charge state to be considered to belong to the
same compound, effectively ignoring this criterion.  This was chosen because
{\color{red} discuss why}, and is implemented by specifying the appropriate
...
compounds with observed abundances for which it is not scientifically plausible
that they might be a compound with an effect on bioactivity level via the
...
and \textit{P. aeruginosa}), as well as several cancer cell lines.  Here, we demonstrate successful detection and
identifiaction of cyO2 thereby validating the use of this statistical analysis
...
determined to be approximately (1047.4898, 3), and (1570.2414, 2) in our data.
...
In this document, the data processing and analysis portion of the data analysis
performed in \papername/ is performed.  The choices that were made when
performing the analysis were explained, and will hopefully provide a template
for laboratories wishing to use methodology for their own research questions.
As a result of the analysis, we see that out of 30,799 MS features originally
measured by the MS-LC process, for each of the eight bioactivity peptide
libraries the pipeline was able to identify the cyO2 compound as one of the
first 100 candidate compounds with a possibility of having deleterious effect on
bioactivity levels, and furthermore as one of the first 10 for three of the
peptide libraries.
