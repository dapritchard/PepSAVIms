...
performed in \papername/.  Providing the analysis will hopefully facilitate a
...
creation of the \textit{Viola odorata} peptide library.  Bioactivity assays
against a panel of microbial pathogens were performed using the generated
peptide library. Each SCX fraction was then subject to nano-LC-ESI-MS/MS (Waters
nanoAcquity UPLC coupled to an AB Sciex TripleTOF 5600) analysis.  After
...
reduction of resazurin.  For anticancer bioactivity, cytotoxicity assays were
...
\[
  \text{percent activity} = \left( 1 - \frac{ \text{RFU of fraction} -
    \text{RFU of positive control} }{ \text{RFU of negative control} -
    \text{RFU of positive control} } \right) \times 100
\]
...
elution include \textit{E. coli} (ec), \textit{E. faecium} (ef),
\textit{A. baumannii} (ab), \textit{P. aeruginosa} (pa), \textit{F. graminearum}
(fg), breast cancer (bc), prostate cancer (pc) and ovarian cancer (oc).
...
parameter to be equal to the run time, which in this case was 60 minutes (of
course any value greater than 60 would have the same effect also).
...
\begin{figure}[p]
  ...
  \caption{\textit{Viola odorata} peptide library bioactivity against all pathogens in which strong activity was seen in the region of cyO2 elution.
    Average % activity values for each fraction are plotted with error bars representing +1 standard deviation. The fractions selected as the region of interest are shown using
    blue bars.
...
prioritization, and identification of cyO2 thereby validating the use of this
statistical analysis approach for bioactive peptide discovery.
...
From the output below we see that for the \textit{E. faecium}, prostate cancer,
and breast cancer data sets, at least one charge state corresponding to cyO2 is
identified in the first 10 candidate compounds.  CyO2 is identified as the 23rd
and 33rd compounds for the \textit{A. baumannii} and \textit{P. aeruginosa} data
sets, respectively, and for the remaining bioactive data sets cyO2 is ranked
...
made when performing the analysis. We hope that in conjunction with the Pipeline
Introduction these vignettes provide a valuable template for laboratories
wishing to implement this methodology for their own research purposes.  As a
result of the analysis, we see that out of 30,799 MS features originally
measured by the LC-MS process, this pipeline was able to identify cyO2 as a
top-10 likely contributor to bioactivity against three of the eight pathogens,
...
\bibitem{kirkpatrick2016} Kirkpatrick et al.  The PepSAVI-MS pipeline for natural
  product bioactive peptide discovery.  {\color{red} finish citation once the
    parts are finalized. Also add citation for elastic net and lasso.}
