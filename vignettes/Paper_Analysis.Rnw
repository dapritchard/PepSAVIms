...
creation of the \textit{V. odorata} peptide library.  Bioactivity assays
...
the processing of this dataset, accurate intact mass and relative intensity
...
Now that the mass spectrometry data is loaded into memory, we begin data processing using
\pkg{\pkgname/} pipeline by consolidating MS features in the data believed to
belong to the same compound.  In this way, all of the abundance
belonging to the same compound will be grouped together and thus resemble a
...
parameter to be equal to the run time, which in this case was 60 minutes (likewise, any value greater than 60 would have the same effect).
...
compounds with observed abundances for which it is unlikely
...
bioactivity data set. Because of the crude nature of SCX, a given peptide should elute
...
\begin{figure}[p]
  ...
  \caption{\textit{Viola odorata} peptide library bioactivity against all
    ...
    region of interest are shown using blue bars.}
