...
% \VignetteIndexEntry{PRISMS Introduction}
...
\begin{center}
  ...
  {\LARGE An introduction to the \prisms/ package}
  \vspace{10mm}
...
The \prisms/ \R/ package provides a collection of software tools with which
to search for candidate biological compounds having a promotive effect on the
reduction of bioactivity levels of an organismal culture.  The package was
...
abbreviated to \papername/) \cite{kirkpatrick2016}.
...
A flowchart for a data analysis performed using \prisms/ is show in figure
\ref{fig: flow chart}.  The blue rectangles and diamond are functions in the
\prisms/ ecosystem; the pink oval denotes mass spectrometry data to be used
...
PRISMS methodology and software.
...
A conceptual presentation of the functions in the \prisms/ package is
...
\end{enumerate}
The region of interest is typically chosen to be a region where a change in the
bioactivity levels is observed; the assumption then is that there are some
compounds eluting during those fractions in the region that are associated with
bioactivity levels.
...
penalty \cite{zou2005}, and tracking the inclusion of the coefficients
corresponding to compounds into the nonzero set along the elastic net path.  An
ordered list of candidate compounds for reduction of bioactivity levels is
obtained by providing the order in which the coefficients corresponding to
compounds entered the nonzero set.
...
\section{An example case: performing the PRISMS pipeline}

In the following section we will illustrate the usage of the core \prisms/
functions by walking through the prototypical PRISMS pipeline.  A flowchart for this prototypical
PRISMS pipeline is shown below.
...
# and charge state of the observations.  The remaining 34 variables are the mass
# spectrometry intensities for across fractions 11 through 43, and fraction 47.
names(mass_spec)
@
...
object \texttt{bioact} is a list containing bioactivity replicates for several
species of viruses and bacteria; we will arbitrarily choose the bioactivity data
for the virus E. coli as an illustrative example, and create an object for this
data from the element in the list.
...
in the \prisms/ pipeline is to to consolidate mass spectrometry observations in
the data that are believed to belong to the same underlying compounds.  This is
...
In this example we specify the data for the mass-to-charge, charge, mass, and
time of peak intensity, and the intensities by specifying the names of the
...
The next step in the \prisms/ pipeline is to remove any potential candidate
...
When using the \texttt{filterMS} function, a first task is to specify the region
...
procedure and their correlation with mean bioactivity levels over replicates.
...
The m/z and charge values of the ranked candidate compounds are available as
elements of the object returned by the \texttt{rankEN} function.  In particular,
the \texttt{"mtoz"} and \texttt{"charge"} elements of the return object are each
numeric vectors providing the m/z and charge values of the ranked candidate
compounds, respectively.

<<ranking-data-compounds>>=
# Print the m/z values for the first 10 candidate compounds
rank_out$mtoz[1:10]

# Print the charge values for the first 10 candidate compounds
rank_out$charge[1:10]
@



% PRISMS objects and class methods ---------------------------------------------
...
in \prisms/ and consider how to further access and manipulate the data
throughout the process.


\subsection{The mass spectrometry data class hierarchy}

The core data structure in the \prisms/ package is the \texttt{msDat}
...
\begin{figure}[H]
  \caption{The \prisms/ class hierarchy}
...
PRISMS pipeline without executing either or both of the \texttt{binMS} or
